...
function EditDoc({ navigation, route }) {
    ...
    const [documentTitle, setdocumentTitle] = useState('Re-upload Document')
    ...
    const addDocument = async () => {
        if (cat.length == 0) {
            ...
            try {
                const response = await FormDataApi.post(`/financemanagement/updatefdoc/`,
                    data);
